    ...
    <div className="flex h-full flex-col">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium transition-colors border-b-2 ${
                  activeType === type ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"
                }`}
              >
                {info.name}
                {cfg && !cfg.is_enabled && (
                  <span className="h-1.5 w-1.5 rounded-full bg-muted-foreground/60" title="Desactive" />
                )}
              </button>
            );
          })}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        {activeConfig ? (
          ...
          <div className="text-sm text-muted-foreground">Configuration non trouvee pour cet agent.</div>
        )}
      </div>
    </div>
  );
