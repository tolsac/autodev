  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">Gerez les repos Git lies a ce projet depuis l'onglet repositories de l'organisation.</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Branche cible par defaut</label>
          <input value={defaultBranch} onChange={(e) => setDefaultBranch(e.target.value)} className={inputClass} />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Template de branche</label>
          <input value={branchTemplate} onChange={(e) => setBranchTemplate(e.target.value)} className={inputClass} />
          <p className="text-[11px] text-muted-foreground/60">Preview : {preview(branchTemplate)}</p>
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Template titre PR</label>
          <input value={prTitle} onChange={(e) => setPrTitle(e.target.value)} className={inputClass} />
          <p className="text-[11px] text-muted-foreground/60">Preview : {preview(prTitle)}</p>
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Template body PR (Markdown)</label>
          <textarea value={prBody} onChange={(e) => setPrBody(e.target.value)} rows={6} className={`${inputClass} h-auto py-2`} />
          <p className="text-[11px] text-muted-foreground/60">Variables : {"{ticket_id}"}, {"{ticket_key}"}, {"{ticket_title}"}, {"{ticket_description}"}, {"{plan_summary}"}</p>
        </div>
