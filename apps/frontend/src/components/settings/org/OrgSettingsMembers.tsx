    ...
    <div className="max-w-2xl space-y-6">
        ...
        {members?.map((m) => (
          <div key={m.id} className="flex items-center gap-3 rounded-lg border border-foreground/5 px-4 py-3">
            ...
            {m.role === "owner" ? (
              ...
              <>
                <select value={m.role} onChange={(e) => updateRole.mutate({ userId: m.user.id, role: e.target.value })} className="h-8 rounded-lg border border-foreground/10 bg-surface px-2 text-xs text-foreground">
                  <option value="admin">Admin</option>
                  <option value="member">Member</option>
                </select>
                <button onClick={() => removeMember.mutate(m.user.id)} className="text-muted-foreground hover:text-destructive">
      ...
      {invitations && invitations.filter((i) => i.status === "pending").length > 0 && (
        <div className="space-y-2">
          <h4 className="text-xs font-medium text-muted-foreground">Invitations en attente</h4>
          {invitations.filter((i) => i.status === "pending").map((inv) => (
            <div key={inv.id} className="flex items-center gap-3 rounded-lg border border-foreground/5 px-4 py-2 text-xs">
              <span className="text-foreground">{inv.email}</span>
              <span className="text-muted-foreground capitalize">{inv.role}</span>
              {inv.is_guest && <span className="rounded bg-foreground/5 px-1.5 py-0.5 text-[10px] text-muted-foreground">Guest</span>}
              <span className="ml-auto text-muted-foreground/60">Expire le {new Date(inv.expires_at).toLocaleDateString("fr-FR")}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {inviteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-md rounded-xl border border-foreground/5 bg-surface-elevated p-6 shadow-2xl space-y-4">
            <h3 className="text-base font-semibold text-foreground">Inviter un membre</h3>
            <input value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="email@example.com" className="h-10 w-full rounded-lg border border-foreground/10 bg-surface px-3 text-sm text-foreground" />
            <select value={inviteRole} onChange={(e) => setInviteRole(e.target.value)} className="h-10 w-full rounded-lg border border-foreground/10 bg-surface px-3 text-sm text-foreground">
              <option value="admin">Admin</option>
              <option value="member">Member</option>
            </select>
            <div className="flex justify-end gap-2">
              <button onClick={() => setInviteOpen(false)} className="h-9 rounded-lg border border-foreground/10 px-4 text-sm text-muted-foreground">Annuler</button>
