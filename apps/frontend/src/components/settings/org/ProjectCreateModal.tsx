...
export default function ProjectCreateModal({ orgSlug, open, onClose }: {
  ...
  const inputClass = "h-10 w-full rounded-lg border border-white/10 bg-[#0c0c14] px-3 text-sm text-foreground placeholder:text-[#8b8b9e] focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 flex w-full max-w-[720px] flex-col rounded-xl border border-white/5 bg-[#13131d] shadow-2xl" style={{ maxHeight: "85vh" }}>
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-[#555566]">Les tickets seront numerotes {prefix || "XX"}-1, {prefix || "XX"}-2...</p>
            </div>
          </section>
          ...
          <section className="space-y-3">
            ...
            <p className="text-xs text-[#8b8b9e]">Liez les repos sur lesquels les agents AI travailleront.</p>
            {(!repos || repos.length === 0) ? (
              <div className="rounded-lg border border-white/5 p-4 text-center text-xs text-[#8b8b9e]">
                Aucun repository disponible. Importez des repos depuis l'onglet Connexions Git.
              </div>
            ) : (
              <div className="space-y-1 rounded-lg border border-white/5 p-2">
                {repos.map((repo) => (
                  <label key={repo.id} className="flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer hover:bg-white/[0.02]">
          ...
          <section>
            <button onClick={() => setGitOpen(!gitOpen)} className="flex w-full items-center gap-2 text-sm font-semibold text-foreground hover:text-primary">
              ...
              Conventions Git
              <span className="text-xs font-normal text-[#555566]">(optionnel)</span>
            </button>
            {gitOpen && (
