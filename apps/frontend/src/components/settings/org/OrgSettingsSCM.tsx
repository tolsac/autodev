...
export default function OrgSettingsSCM({ orgSlug }: { orgSlug: string }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {connLoading ? (
        <div className="text-sm text-[#8b8b9e]">Chargement...</div>
      ) : !connections || connections.length === 0 ? (
        <div className="rounded-lg border border-white/5 p-8 text-center">
          <svg className="mx-auto size-8 text-[#555566]" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>
          <p className="mt-3 text-sm text-[#8b8b9e]">Aucune forge connectee</p>
        ...
        <div className="space-y-2">
          {connections.map((conn) => (
            <div key={conn.id} className="flex items-center justify-between rounded-lg border border-white/5 px-4 py-3">
              ...
              <button onClick={() => disconnect.mutate(conn.id)} className="h-7 rounded-md border border-white/10 px-2.5 text-xs text-[#8b8b9e] hover:border-destructive/30 hover:text-destructive">Deconnecter</button>
            </div>
          ))}
        </div>
      )}
      ...
      {connections && connections.length > 0 && (
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setImportOpen(true)} className="h-8 rounded-lg border border-white/10 px-3 text-xs text-[#8b8b9e] hover:text-foreground">+ Importer</button>
          </div>

          {reposLoading ? (
            <div className="text-sm text-[#8b8b9e]">Chargement...</div>
          ) : !repos || repos.length === 0 ? (
            <div className="rounded-lg border border-white/5 p-6 text-center text-sm text-[#8b8b9e]">
            ...
            <div className="space-y-1">
              {repos.map((repo) => {
                ...
                return (
                  <div key={repo.id} className="flex items-center gap-3 rounded-lg border border-white/5 px-4 py-2.5">
                    <svg className="size-4 flex-shrink-0 text-[#8b8b9e]" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>
